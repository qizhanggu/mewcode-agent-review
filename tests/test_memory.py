...
from localdesk.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
)
from localdesk.memory.auto_memory import MemoryManager
from localdesk.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from localdesk.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        localdesk_md = tmp_path / "LOCALDESK.md"
        localdesk_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """对齐 Go 版发现顺序：LOCALDESK.md 在前，.localdesk/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "LOCALDESK.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".localdesk"
        dotdir.mkdir()
        # Go 版不发现 .localdesk/LOCALDESK.md，只发现 .localdesk/INSTRUCTIONS.md（legacy）
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".localdesk" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".localdesk" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".localdesk" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "localdeskMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "localdeskMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型与 Go 版一致。"""
        from localdesk.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
