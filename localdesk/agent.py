...
from localdesk.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from localdesk.conversation import ThinkingBlock as ConvThinkingBlock
...
from localdesk.hooks import HookContext, HookEngine, ToolRejectedError
from localdesk.hooks.engine import HookNotification
from localdesk.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from localdesk.tools import ToolRegistry
from localdesk.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".localdesk" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".localdesk" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from localdesk.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from localdesk.context.manager import (
