...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".localdesk" / "config.yaml",
        cwd / ".localdesk" / "config.yaml",
        cwd / ".localdesk" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .localdesk/config.yaml "
            "in project or ~/.localdesk/config.yaml"
        )
    return merged
