...
from localdesk.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from localdesk.worktree.models import Worktree, WorktreeSession
from localdesk.worktree.session import load_worktree_session, save_worktree_session
from localdesk.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".localdesk" / "worktrees"
        )
        self._localdesk_dir = Path(repo_root) / ".localdesk"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._localdesk_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._localdesk_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._localdesk_dir)
        if session is None:
            return None
