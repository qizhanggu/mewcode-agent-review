...
def _session_path(localdesk_dir: Path) -> Path:
    return localdesk_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(localdesk_dir)
    if session is None:
...
def load_worktree_session(localdesk_dir: Path) -> WorktreeSession | None:
    path = _session_path(localdesk_dir)
    if not path.exists():
        return None
    try:
