...
from localdesk.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from localdesk.commands.completion import CompletionPopup
from localdesk.commands.handlers import register_all_commands
from localdesk.config import MCPServerConfig, ProviderConfig
from localdesk.hooks import HookContext, HookEngine, load_hooks
from localdesk.conversation import ConversationManager, Message
from localdesk.mcp import ConnectResult, MCPManager
from localdesk.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from localdesk.agents.notification import inject_task_notifications
from localdesk.commands.handlers.tasks import create_tasks_command
from localdesk.skills.executor import SkillExecutor
from localdesk.skills.loader import SkillLoader
from localdesk.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from localdesk.cache import FileCache
from localdesk.tools import ToolRegistry, create_default_registry
from localdesk.tools.agent_tool import AgentTool
from localdesk.tools.ask_user import AskUserEvent, AskUserTool
from localdesk.tools.impl.tool_search import ToolSearchTool
from localdesk.tools.install_skill import InstallSkillTool
from localdesk.tools.load_skill import LoadSkill
from localdesk.worktree.cleanup import start_stale_cleanup_task
from localdesk.worktree.manager import WorktreeManager
from localdesk.commands.handlers.worktree import create_worktree_command
from localdesk.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".localdesk", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".localdesk" / "history"
...
class LocalDeskApp(App):
    ...
    def __init__(
        ...
        from localdesk.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_LOCALDESK_THEME)
        self.theme = "localdesk"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        if self._sandbox_cfg.enabled:
            from localdesk.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.localdesk/config.yaml",
                        f"{work_dir}/.localdesk/permissions.local.yaml",
                    ],
        ...
        from localdesk.filehistory import FileHistory
        ...
        from localdesk.tools.exit_plan_mode import ExitPlanModeTool
        ...
        from localdesk.tools.enter_worktree import EnterWorktreeTool
        from localdesk.tools.exit_worktree import ExitWorktreeTool
        ...
        from localdesk.commands.handlers.trace import create_trace_command
        ...
        from localdesk.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from localdesk.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from localdesk.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from localdesk.plan_dialog import InlinePlanWidget, PlanChoice
        from localdesk.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from localdesk.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from localdesk.permission_dialog import InlinePermissionWidget
