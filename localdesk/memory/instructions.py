...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.localdesk/LOCALDESK.md, ~/.localdesk/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 LOCALDESK.md 和 AGENTS.md
    3. workDir/.localdesk/INSTRUCTIONS.md（遗留格式）
    4. workDir/LOCALDESK.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".localdesk" / "LOCALDESK.md")
    _add(home / ".localdesk" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".localdesk" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "LOCALDESK.local.md")
